...
Our algorithm is inspired by the classical Hamiltonian Monte Carlo (HMC) algorithm, a modification of the original Metropolis-Hastings algorithm that eliminates random walk behavior. It does so by extending the state space of the system of interest to include momentum variables and then utilizes time-evolution of the extended Hamiltonian to transition to new states, which further eliminates the need for rejection sampling theoretically. This allows us to avoid the dealbreakers of Marriot-Watrous rewinding \cite{temme2011} or other weak measurement schemes \cite{jiang2024quantummetropolissamplingweak} present when trying to adapt standard Metropolis-Hastings algorithms to quantum computers.

The channel describing the resulting algorithm is as follows. First, prepare the system in a state diagonal in the Hamiltonian basis, the maximally mixed state or a Haar random pure state are sufficient, and a single ancilla qubit in a thermal state $\frac{e^{-\beta H_E}}{1 + e^{-\beta \gamma}}$, with an energy gap $\gamma$ that can be chosen uniformly at random from the interval $[0, 4\norm{H_S}]$. Then, choose an interaction term with Haar random eigenvectors and I.I.D Gaussian eigenvalues and add it to the system and environment Hamiltonians with a coupling constant $\alpha$. Finally, simulate the system-environment Hamiltonian wth interaction for a time $t$ and then trace out the ancilla qubit. This yields our overall channel
\begin{equation}
...
Our manuscript also provides extensive numerics that explore various behaviors of the channel $\Phi$, a few of which we replicate here. For small harmonic oscillator instances we are able to study the $\beta$ and $\epsilon$ dependence of the channel, as seen in Fig. \ref{fig:harmonic_extended_abstract}. The $\beta$ dependence of the total simulation time exhibits unexpected behavior even for $\dim_S = 4$ truncations, where it appears that the spectral gap of the Markov chain \emph{increases} with $\beta$, meaning ground states can be prepared faster than intermediate temperatures when starting from the maximally mixed state. When probing the $\epsilon$ scaling of the algorithm we push the algorithm in regimes beyond prescribed values of $\alpha$ and $t$ as given by our weak-coupling expansion. We find that at $\epsilon = 0.005$ the total simulation time can be reduced by over 4 orders of magnitude. 

\begin{figure}
    \centering
    \begin{subfigure}{0.49\linewidth}
    \includegraphics[width=\textwidth]{numerics/data/epsilon_fitting_4.pdf}
    \caption{Scaling of total simulation time with respect to $\epsilon$ for our analytic prescription of $\alpha \in O(1/t^3)$ in addition to conjectured scalings of $\alpha \in O(1/t^2)$ and $\alpha \in O(1/t)$.} 
    \end{subfigure}
    \begin{subfigure}{0.49\linewidth}
    \includegraphics[width=\textwidth]{numerics/data/sho_total_time_vs_beta_dim_4.pdf}
        \caption{Total simulation time scaling with respect to $\beta$ for fixed $\alpha, t, $ and $\epsilon$ for a fixed starting state of $\identity / \dim_S$ for $\dim_S = 4$. }
    \end{subfigure}
    \caption{Harmonic oscillator numerics.}
    \label{fig:harmonic_extended_abstract}
\end{figure}

Beyond small harmonic oscillator instances we further numerically explore small Hydrogen chains which are typical quantum chemistry benchmark systems. In Figure \ref{fig:h2_error} we show the error scaling with respect to the number of interactions for a pair of hydrogen atoms for various $\alpha$ and $t$ settings and again observe that strong coupling cools much faster but has a higher error floor. In practice, one would want to start with large values of $\alpha$ that decrease over time. In Figure \ref{fig:h3_noise} we quantify the effects of noise on our Perfect Knowledge results for a chain of 3 Hydrogen atoms. This plot is generated by sampling values of $\gamma$ from the exact eigenvalue spectrum and then adding Gaussian noise to the sample. The total simulation time needed to thermalize is plotted as a function of the width of this added noise, demonstrating an even interpolation from the scenario with a priori eigenvalue knowledge to no uniform sampling across the eigenvalue spectrum.


Due to space constraints, we are unfortunately unable to offer comprehensive comparisons with existing Gibbs sampling algorithms \cite{chen2023quantumthermalstatepreparation, ding2024efficientquantumgibbssamplers, zhang2023dissipative, shtanko2023preparingthermalstatesnoiseless, jiang2024quantummetropolissamplingweak, gilyen2024quantumgeneralizationsglaubermetropolis, temme2011, poulin2009sampling} and instead point the reader to Section I of the full manuscript for additional details. We remark that our algorithm is most similar to the algorithm by Shtanko and Movassagh \cite{shtanko2023preparingthermalstatesnoiseless} but with a very different interaction model. This allows us to not only extend the range of systems applicable, but allows us to compute the explicit runtime in the ground state limit as $\beta \to \infty$, a major open problem for Linbladian based approaches. These are two open problems posed in \cite{shtanko2023preparingthermalstatesnoiseless} which we resolve. We remark that compared to many of the Linbladian inspired algorithms \cite{gilyen2024quantumgeneralizationsglaubermetropolis, ding2024efficientquantumgibbssamplers} our algorithm suffers from worse $\epsilon$ scaling and contains explicit dimensionful factors, but does offer a drastically simpler circuit construction with potential for improved analysis and has computable mixing times in the ground state limit. Lastly, we thank the reader for their time and attention in reviewing our submission.
 \begin{figure}[H]
    % \begin{subfigure}
    % \end{subfigure}
    ...
    \label{fig:h3_noise}
    \end{subfigure}
    \caption{Hydrogen Chain Numerics}
\end{figure}
