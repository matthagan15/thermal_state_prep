...
\begin{align}
    \int \bra{i,j} \frac{\partial^2}{\partial \alpha^2} \Phi_G(\rho) \bigg|_{\alpha = 0} \ket{k,l} dG &= t^2 \sigma^2 \sum_{i',j',m,n} C_E \frac{\delta(i, j | k,l)}{\dim (\dim + 1)} (1 + \delta(i,j | m,n)).
\end{align}
...
\begin{align}
    ...
    &~ ~ - 2 \rho_{i,j} \parens{\int_0^1 \int_0^1 e^{i (E_{i,j} - E_{m,n})s_1 (1- s_2)t}s_1 ds_1 ds_2 + \int_0^1 \int_0^1 e^{-i(E_{i,j} - E_{m,n})s_2(1-s_1)t}((1-s_1)ds_1 ds_2)} \\
...
Note that this expression is indeterminate whenever $m = i, n = j$. For this simply note that each of the phases in the exponents of the integrals are 0, leading to $C_E = 0$. This means we can restrict ourselves to $m \neq i, n \neq j$. 

\begin{align}
    \int \bra{i,j} \frac{\partial^2}{\partial \alpha^2} \Phi_G(\rho) \bigg|_{\alpha = 0} \ket{i,j} dG &= \frac{2 \sigma^2}{\dim + 1} \sum_{(m,n) \neq (i,j)} \frac{(2 \rho_{m,n} - \rho_{i,j}) - 2(\rho_{m,n} - \rho_{i,j}) \cos ((E_{i,j} - E_{m,n})t)}{(E_{i,j} - E_{m,n})^2}
\end{align}

Now lets look at the specific system of a truncated harmonic oscillator, which implies $E_{i,j} = \hbar \omega (1 + i + j) = 1 + 1 + j$, as we are not concerned with the specific scaling of $\omega$. This gives us the thermal state of $\rho = \sum_{i,j} \ketbra{i}{i} \otimes \ketbra{j}{j}\rho_{i,j} = \frac{e^{-\beta (1 + i + j)}}{\sum_{e^{-\beta(1+  i + j)}}} \ketbra{i}{i} \otimes \ketbra{j}{j}$
...
% \end{equation}
% Plugging these two expressions into Eq. \eqref{eq:second_order_duhamel_factored}, performing the integration over $dG = dD dU_G$ and summing over $k', l'$ we get
% \begin{align}
%     2 t^2 \sigma^2 \sum_{i',j',m,n} \bra{m,n} \rho \ket{m,n} \int_0^1 \int_0^1 e^{i (E_{i,j} - E_{m,n}) s_1 t}  e^{i (E_{m,n} - E_{k,l})s_2 t} ds_1 ds_2 \nonumber \\ 
%     \times \int \bra{i,j} U_G \ket{i',j'} \bra{i',j'} U_G^\dagger \ket{m,n} \bra{m,n} U_G \ket{k',l'} \bra{k',l'} U_G^\dagger \ket{k,l} dU_G \\
%     =2 t^2 \sigma^2 \sum_{i',j',m,n} \bra{m,n} \rho \ket{m,n} \frac{e^{i(E_{i,j} - E_{m,n})t} - 1}{(E_{i,j} - E_{m,n})t} \frac{e^{i(E_{m,n} - E_{k,l})t} - 1}{(E_{k,l} - E_{m,n})t} \frac{\delta(i,j | k,l)}{\dim^2 - 1} \parens{\delta(i,j | m,n) + \frac{\dim - 1 - \delta(i,j | m,n)}{\dim}}
% \end{align}
% \begin{align}
