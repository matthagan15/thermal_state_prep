...
we start with a trotter decomposition. We then write the trace as a schatten norm. then $\diamondnorm{\Phi^{\circ r}} \leq r \diamondnorm{\Phi}$
The trotter decomposition can be written as
\begin{equation}
    e^{i(H + \alpha G)t} = \parens{e^{i G \alpha t/r} e^{i H t/r}}^r + E_{trott}.
\end{equation}
where we can choose $r$ such that $\norm{E_{trott}}_{p} < \epsilon$. 
\begin{equation}
    \norm{A}_p := \trace{\parens{AA^*}^{p/2}}^{1/p}
\end{equation}
\begin{align}
    \trace{\int U_G H_{sys} U_G^* ~ \rho ~ dG } &= \int \trace{}
\end{align}
We are going to have to require that $H_{sys}$ be positive semidefinite. This is because if the product $H_{sys} \sigma$ for some density matrix $\sigma$ is positive semidefinite then we can write $H_{sys} U_{G}^* \rho U_G = H_{sys} \sigma = A^*A$ for some $A$. 
If $[H_{sys}, \sigma] = 0$ then $H = (H_{sys}^{1/2} \sigma^{1/2})^* (H_{sys}^{1/2} \sigma^{1/2})$ If we do this then we have
\begin{align}
    \trace{H_{sys} \sigma} &=  \norm{(H_{sys}^{1/2} \sigma^{1/2})^{2/p}}_{p}^p \\ 
    &= \norm{(H_{sys} \sigma)^{1/p}}_p^p
\end{align}
\begin{align}
    \trace{e^{i(Ht/r) }}
\end{align}
