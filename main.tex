...
\end{align}
 ...
 \begin{align}
    ...
    &~ +\frac{2\sigma^2 e^{i \Delta_{ab}t}}{\dim+1} \parens{\delta_{ab} \sum_{i: \Delta_{ai} \neq 0 } \frac{2(1- \cos (\Delta_{ai}t))}{\Delta_{ai}^2} \ketbra{i}{i} + t^2 \delta_{ab} \sum_{i : \Delta_{ai} = 0} \ketbra{i}{i}}
 \end{align}

Note that this allows us to easily show that a diagonal entry does not transition to an off-diagonal entry at this order in $\alpha$, so we assume $c \neq d$ in the following:
\begin{align}
...
We can further look at transitions from energy diagonal basis within the degenerate subspace. So we want to analyze the second order correction to $\bra{b} \int \Phi_G(\ketbra{a}{a}) dG \ket{b}$ such that $\lambda(a) = \lambda(b)$ but $a \neq b$:
\begin{align}
    \lambda(a) = \lambda(b) \implies \bra{b} \int \frac{\partial^2}{\partial \alpha^2} \Phi_G(\ketbra{a}{a}) \bigg|_{\alpha = 0} dG \ket{b} &= \frac{\sigma^2 \alpha^2 t^2}{\dim + 1}.
\end{align}
Now if we look at transitions between energy subspaces, so $\lambda(b) \neq \lambda(a)$ then we have
\begin{equation}
    \lambda(b) \neq \lambda(a) \implies \bra{b} \int \frac{\partial^2}{\partial \alpha^2} \Phi_G(\ketbra{a}{a}) \bigg|_{\alpha = 0} dG \ket{b} = \frac{2\sigma^2 \alpha^2}{\dim + 1} \frac{1 - \cos( \Delta_{ab} t)}{\Delta_{ab}^2},
\end{equation}
which we notice approaches the degenerate case as $\Delta_{ab} \to 0$. Finally we compute the self-loop term $\ketbra{a}{a} \to \ketbra{a}{a}$
\begin{align}
    a = b \implies \bra{a} \int \frac{\partial^2}{\partial \alpha^2} \Phi_G(\ketbra{a}{a}) \bigg|_{\alpha = 0} dG \ket{a} = -\frac{ \sigma^2 \alpha^2}{\dim + 1} \parens{\sum_{j: \Delta_{aj} \neq 0} \frac{2( 1- \cos (\Delta_{aj}t)) }{\Delta_{aj}^2} + t^2 (\eta(a) - 1)}.
\end{align}
We can then see that by summing over all diagonal outputs that the second order correction is traceless, implying that to this order in $\alpha$ the channel preserves trace (the $\bigo{\alpha^0}$ correction is simply the channel itself). 
