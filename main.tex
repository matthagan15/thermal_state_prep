...
\begin{abstract}
    We present a new quantum algorithm for preparing arbitrary thermal states on a digital quantum device with a single ancilla qubit. For preparing ground states our algorithm demonstrates a quantifiable advantage for exact knowledge of eigenstate differences of the system $\lambda_S(i) - \lambda_S(j)$ compared to complete ignorance, with the ratio scaling as $\widetilde{\Theta}\frac{}{}$
\end{abstract}
\tableofcontents
...
\end{enumerate}
\end{definition}
We then use this categorization to aid in the proof of the following theorem which gives the repeated interaction channel's transitions to second order.
...
\begin{theorem}(Markov Relaxation Time) \label{thm:markov_chain_bound}
    Let $M : \mathbb{R}^{N} \to  \mathbb{R}^{N}$ be a Markov transition matrix acting on an $N$ dimensional state space with absolute spectral gap $\lambda_{\star} \coloneqq 1 - \max_{i > 1} |\lambda_i(M)|$, where the eigenvalues of $M$ are ordered $1 = \lambda_1 \ge \lambda_2 \ge \ldots \ge \lambda_N \geq -1$. Given this gap, if the number of steps $L$ in the Markov chain satisfies the following bound
    \begin{align}
        L \ge \frac{N}{\lambda_{\star}} \left( 2\log \frac{1}{\lambda_{\star}} + 4(1 + \log 2) +  \frac{1}{N} (2 \log \left( \frac{1}{\epsilon} \right) - 1) \right) ,
    \end{align}
    then the resulting state, $M^L \vec{x}$, is $\epsilon$ close to the fixed point
    \begin{equation}
...
% In addition to knowing how many interactions will be needed for the Markov chain to relax we will need to know what states are fixed points of the dynamics.
% \begin{lemma}[Markov Fixed Point]\label{lem:fixed_point}
%     Let $M : \hilb_S \to \hilb_S = I + T$ be a Markov chain with transition matrix $T = $ with real entries and $\trace{T} = 0$. The fixed point $\vec{p}$ can be expressed as
...
In this section we analyze the dynamics of the weak-coupling expansion from Section \ref{sec:weak_coupling} as a thermal state preparation routine for specific systems. We start with just a single qubit, which is already an area of active research in the open quantum systems field. As the resulting Markov transition matrix is $2 \times 2$. 
...
We then plug this in to the Markov Relaxation Theorem \ref{thm:markov_chain_bound} to get $L \in \widetilde{\Omega}\left( \frac{\dim}{\widetilde{\alpha}^2}\right)$. We then use the remainder bound in Lemma \ref{thm:remainder_bound} and the off-resonance norm to set values for $\alpha$ and $t$. First $\delta_{\min} = \min_{\Delta(i,j) \neq \Delta(k,l)} |\Delta(i,j) - \Delta(k,l)| = |2 \Delta - \Delta| = \Delta$.
\begin{align}
...
\begin{equation}
    L t \in \bigotilde{\frac{\dim^2}{(\alpha t)^2} t} = \bigotilde{ \dim^2 \Delta^4 t^5} = \widetilde{O} \parens{\frac{\dim_S^{9.5}}{\Delta \epsilon^{2.5}} }.
\end{equation}
\end{proof}
...
\begin{theorem}[Generic System Thermal State Fixed Point]
    ...
    \end{align}
    %Combined with Jerison's Markov Relaxation Theorem \ref{thm:markov_chain_bound} this implies the existence of sufficiently 
...
\begin{proof}
From Definition~\ref{def:resonance} we see that for $j > i$ we can write for $q(1;i,j)$ the coefficient for the on-resonance term with $j>i$, \begin{equation}
    \bra{j} \mathcal{T}_{\on}(\ketbra{i}{i}) \ket{j} = \widetilde{\alpha}^2 q(1; i, j) \mathbf{I}[\gamma = \Delta(i,j)]
\end{equation} and for $j < i$ we have then write for brevity the result in terms of $q(0;i,j)$ 
...
%\textcolor{blue}{NW: There are a couple of steps here that seem to be skipped involving the substitution and it would be great if we could add them in.}
\begin{align}
...
\begin{align}
    ...
    &\leq \frac{e^{\beta \delta_{\min}}}{4} \int_{-\delta_{\min}t/2}^{\delta_{\min}t/2} \sinc^2(u) du \\
    &\leq e^{\beta \delta_{\min}} \frac{\pi }{4}.
\end{align}
Plugging in to Eq. \eqref{eq:thermal_state_detailed_balance} we have 
\begin{equation}
    \left| e^{-\beta \Delta_S(i ,j)} e_j^T \EE_\gamma T_\gamma e_i - e_i^T \EE_\gamma T_\gamma e_j \right| \leq \frac{\alpha^2 t e^{\beta \delta_{\min}}}{\dim + 1} \frac{\pi}{2}.
\end{equation}
...
\begin{align}
    ...
    &\leq (\dim_S - 1) \frac{\alpha^2 t e^{\beta \delta_{\min}}}{\dim + 1} \frac{\pi}{2} \\
    &\leq \alpha^2 t e^{\beta \delta_{\min}} \frac{\pi}{2}.
\end{align}
...
\begin{align}
    \norm{\EE_\gamma T_\gamma \vec{p}_\beta}_1 &= \sum_j \abs{e_j^T \EE_\gamma T_\gamma \vec{p}_\beta } \leq \sum_j \alpha^2 t e^{\beta \delta_{\min}} \frac{\pi}{2} = \alpha^2 t e^{\beta \delta_{\min}} \dim_S \frac{\pi}{2}.
\end{align}
...
\begin{lemma}\label{lem:ground_state_transition_upper_triangular}
    Let $H_S$ be a non-degenerate Hamiltonian and let $\alpha, t, $ and $\gamma$ be arbitrary positive real numbers. In the limit as $\beta \to \infty$, the Markov transition matrix $I + T$ associated with the thermalizing channel $\Phi$ is upper triangular
    \begin{equation}
        i > j \implies e_i^T T e_j = 0,
    \end{equation}
     and the ground state is a fixed point of the dynamics
     \begin{equation}
         (I + T) e_1 = e_1.
     \end{equation}
\end{lemma}
\begin{proof}
    To show that $I + T$ is upper triangular we just need to show that $T$ is upper triangular. We can show this using Theorem \ref{thm:second_order_transition} which gives us the on-resonance transition amplitude. We assume $i < j$, which implies $\Delta_S(i,j) \le 0$, and get
    \begin{align}
    ...
    \begin{align}
        \lim_{\beta \to \infty} e_1^T T e_1 &= \lim_{\beta \to \infty} \bra{1} \TT_{\on}(\ketbra{1}{1}) \ket{1} \\
        &= - \lim_{\beta \to \infty} \sum_{j > 1} \bra{j} \TT_{\on}(\ketbra{1}{1}) \ket{j} \\
        &= - \sum_{j > 1} \lim_{\beta \to \infty}  \bra{j} \TT_{\on}(\ketbra{1}{1}) \ket{j} \\
        &= 0.
    \end{align}
    Since we already showed that $e_j^T T e_1 = 0$ for $j > 1$, it follows that $T e_1 = 0$ and that
    \begin{equation}
        (I + T) e_1 = e_1,
    \end{equation}
    showing the ground state is fixed.
\end{proof}
...
Using Lemma \ref{lem:ground_state_transition_upper_triangular} we know that $\EE_\gamma T_\gamma$ is upper triangular and that $e_1$ is a fixed point. As this proof holds for arbitrary $\gamma$ it also holds for the expected transition matrix. We now need to compute what the diagonal entries are exactly, as these directly encode the spectrum of the Markov chain. As Lemma \ref{lem:ground_state_transition_upper_triangular} shows that $\EE_\gamma e_1^T T_\gamma e_1 = 0$ we can compute the $i > 1$ diagonal entries as
\begin{align}
...
\begin{proof}
    ...
    \end{equation}
    Following the proof of Theorem \ref{thm:ground_state} this gives a spectrum of 0 and $- 2\frac{\widetilde{\alpha}^2}{\dim_S (\dim_S - 1)} \sum_{j < i} \eta(i,j)$ for all $i$. As the spectral gap of $\EE_\gamma T_\gamma$ is the same as the spectral gap of $I + \EE_\gamma T_\gamma$,
...
\subsection{Numeric Experiments} \label{sec:numeric_experiments}
...
In this section we present the more technical work needed to state our results in Section \ref{sec:weak_coupling}. The first two results are Lemmas that compute the effects of the randomized interaction in a form that are usable in the main result of Lemma \ref{lem:big_one}.
\matt{cite this}
...
We further will use the above results to prove Theorem~\ref{thm:remainder_bound} which yields a 
