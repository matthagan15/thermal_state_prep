...
\begin{abstract}
    We present a new quantum algorithm for preparing arbitrary thermal states on a digital quantum device with a single ancilla qubit. The algorithm involves preparing our ancilla in the thermal state for the desired $\beta$, simulating the system-ancilla combination with a randomized interaction, followed by refreshing the state of the ancilla. This algorithm does not contain any accept-reject stages or eigenvalue filtering functions and exhibits significant speedups when knowledge of eigenvalue differences are incorporated. We demonstrate analytic asymptotic speedups with exact knowledge of the eigenvalue differences $\lambda_S(i) - \lambda_S(j)$ of the system for ground state preparation and numeric evidence of speedups for finite $\beta$ (nonzero temperature) for small Hydrogen chains. Our main result is a runtime bound of ground state preparation ($\beta \to \infty$) for arbitrary non-degenerate Hamiltonians with or without knowledge of eigenvalue differences. For finite $\beta$ we are able to show that the thermal state is the unique fixed point of an arbitrary system with exact eigenvalue difference knowledge and an approximate fixed point with no eigenvalue difference knowledge. We bound the runtime of finite $\beta$ state preparation in terms of the spectral gap of a Markov chain. We supplement our theoretic analysis of the algorithm with expansive numerics.
\end{abstract}
\tableofcontents
...
In this section we analyze the dynamics of the weak-coupling expansion from Section \ref{sec:weak_coupling} as a thermal state preparation routine for specific systems. We start with just a single qubit in Section \ref{sec:single_qubit}, which is already a complex enough of a scenario that there is active research on this setting in the open quantum systems field \cite{prositto2025equilibrium}. For a single qubit we are able to give asymptotic runtime bounds for preparing arbitrary $\beta$ thermal states with specific values of $\alpha$ and $t$ to guarantee a trace distance error of $\epsilon$ from the thermal state. Further, our results hold without exact knowledge of the system energy gap $\Delta$. We do require knowledge of a window of width $2 \sigma$ that contains $\Delta$, and our runtime is parametrized in terms of $\sigma$. 

After we develop the theory for a single qubit we extend our sights to a truncated harmonic oscillator in Section \ref{sec:harmonic_oscillator}. This section can be viewed as a warmup to the proofs that work for general systems in Section \ref{sec:general_systems} but does contain some unique insights by itself. 
...
\subsection{Harmonic Oscillator - DEFCON 3} \label{sec:harmonic_oscillator}
...
\subsection{Ground State Preparation - DEFCON 5}
