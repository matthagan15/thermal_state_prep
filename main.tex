...
\begin{proof}
    ...
    \end{align}
    By plugging in Eqs. \eqref{eq:on_resonance_i_geq_j} and \eqref{eq:on_resonance_i_leq_j} we are done with the self-transition terms. The last expression needing proof is the bound on the trace norm of the off-resonance map. For this we will assume a worst-case scenario in which $\gamma$ forces all 3 terms in Eq. \eqref{eq:t_2_intermediate_1} to be off-resonant. Then using Lemma \ref{lem:sinc_poly_approx} to bound the $\sinc$ factors gives us
    \begin{align}
        i \neq j \implies \abs{\bra{j}\mathcal{T}_{\off}(\ketbra{i}{i})\ket{j}} &\le \widetilde{\alpha}^2\frac{4}{\delta_{\min}^2 t^2} \left( 1 + q(0) + q(1)\right) = \frac{8 \alpha^2}{\delta_{\min}(\dim + 1)}.
    \end{align}
    This allows us to bound the off-resonance self-transition term
    \begin{align}
        \abs{\bra{i}\mathcal{T}_{\off}(\ketbra{i}{i})\ket{i}} &= \abs{- \sum_{j \neq i} \bra{j} \mathcal{T}_{\off}(\ketbra{i}{i}) \ket{j}} \le (\dim_S - 1) \frac{8 \alpha^2}{\delta_{\min} (\dim + 1)} \le \frac{4 \alpha^2}{\delta_{\min}}.
    \end{align}
    ...
    \begin{align}
        ...
        &\le \sum_{i} \rho_{i,i} \left( (\dim_S - 1) \frac{8 \alpha^2}{\delta_{\min}(\dim + 1)} + \frac{4 \alpha^2}{\delta_{\min}} \right) \\
        &\le \frac{8 \alpha^2}{\delta_{\min}}.
    \end{align}
...
\begin{theorem} \label{thm:single_qubit}
    Let $H_S$ be an arbitrary single qubit Hamiltonian with eigenvalue gap $\Delta$ and $\rho$ any input state diagonal in the $H_S$ basis, $[\rho, H_S] = 0$. After $L$ interactions with $\alpha$ and $t$ satisfying the following bounds the state of the system will be $\epsilon$ close in trace distance to the thermal state at temperature $\beta$
    \begin{align}
        \norm{\rho_S(\beta) - \Phi^{\circ L}(\rho)}_1 \in \bigotilde{\epsilon}.
    \end{align}
    The lower bound for $L$ is
    \begin{equation}
        L \ge \frac{2}{\widetilde{\alpha}^2} \left( 2 \log \frac{1}{\widetilde{\alpha}^2} + 4( 1 + \log 2) - \frac{1}{2} + \log \frac{2}{\epsilon} \right)
    \end{equation}
    or $L \in \widetilde{\Omega}\left( \frac{1}{\widetilde{\alpha}^2} \right)$. We can then set $\alpha = 1 / (\Delta^2 t^3)$ and require $t \in \widetilde{\Omega}\left( \frac{\sqrt{\dim}}{\Delta \sqrt{\epsilon}} \right)$ in order for the error to be in $\widetilde{O}(\epsilon)$. The bound properly carried through to $L$ is $L \in \widetilde{\Theta}\left(\frac{\dim^3}{\epsilon^2} \right)$. Therefore the total simulation time required scales like $\widetilde{\Theta}\left( \frac{\dim^{7/2}}{\Delta \epsilon^{5/2}} \right)$. \matt{This is garbage, but is correct garbage. Can possibly be improved through the error analysis. Instead of setting $\alpha = 1/\Delta^2 t^3$ to pull out the same asymptotic parameters we could do something a bit smarter.}
\end{theorem}
\begin{proof}
    ...
    We first need to compute the entries of $T$, which are given by the on-resonance terms. As there are only 2 states for the system $T$ is a $2 \times 2$ matrix. Using Theorem \ref{thm:on_and_off_resonance} these 4 entries are given as
    \begin{align}
...
Now that we have the fixed point and convergence rate for a single qubit by precisely tuning our environment $\gamma = \Delta$ we would like to know if this result is robust to deviations of $\gamma$ about $\Delta$. We will consider $\gamma$ a stochastic variable with probability $1 - \delta$ of being within a window about $\Delta$ of width \matt{Fill in window width}. We MAY also need that $\gamma$ is an unbiased estimator to the mean, or $\mathbb{E}[\gamma] = \Delta$, but not clear right now. To go about this we will reintroduce the factors of $\sinc$ into the on-resonance terms from the previous analysis and then propagate these factors through to the eigenvalues. Once we know the eigenvalues we can use a brief analysis of $\sinc$ to provide bounds on the gap with a known probability. 
...
\end{align}
 ...
 We find this window by computing a Taylor Series of $1 / (1 + e^{-\beta \gamma})$ about $\gamma$. For our single qubit state the trace distance can be computed
 \begin{align}
 ...
 \end{align}
    Now we expand this about $\gamma = \Delta$
    \begin{align}
    \frac{1}{1 + e^{-\beta \gamma}} &= \frac{1}{1 + e^{-\beta \Delta}} + (\gamma - \Delta) \beta \frac{1}{1 + e^{-\beta \gamma_{\star}}} \frac{e^{-\beta \gamma_{\star}}}{1 + e^{-\beta \gamma_{\star}}},
    \end{align}
 which leads to the distance as $\norm{\rho_S(\beta, \gamma) - \rho_S(\beta, \Delta)}_1 \le 2 \beta |\Delta - \gamma|$.
If we want our distance to be less than $\epsilon$ we require 
\begin{equation}
    \abs{\gamma - \Delta} \le \frac{\epsilon}{2 \beta}. \label{eq:single_qubit_ineq_1}
\end{equation}

Now we move on to bounding the number of interactions needed to reach the fixed point of the Markov chain. As seen in the proof of Theorem \ref{thm:single_qubit}
