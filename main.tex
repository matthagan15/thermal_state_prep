...
\begin{definition} \label{def:separated_hamiltonians}
...
As we see above, the thermal state $\rho_S(\beta_E)$ is an approximate fixed point The next step we need to prove is to find the range of temperatures for which the system is \emph{ not} in an approximate fixed state. This means
\begin{claim}
   Let $\delta = \beta_E - \beta$ and assume $H_S$ is well separated per Def. \ref{def:separated_hamiltonians}. Further, let $\gamma$ be $\Delta_{\sinc}$ close to the energy difference $\Delta_S(u,l)$. If $\delta \geq \delta_{LB}$ then the thermal state is not an approximate fixed point
   \begin{equation}
    \norm{\rho_S(\beta) - \Phi^{(2)}(\rho_S(\beta))}_2^2 \geq \epsilon_{fix}.
   \end{equation}
\end{claim}
\begin{proof}
...
\begin{theorem}
    ...
    \begin{equation}
        \beta_{LB} \leq \beta \leq \beta_{UB} \implies \norm{\rho_S(\beta_E) - \Phi(\rho_S(\beta))}_2^2 \leq \epsilon_{fix}.
    \end{equation}
