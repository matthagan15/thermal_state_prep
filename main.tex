...
\begin{lemma}{lemma}\label{lem:sinc_poly_approx}
    Let $f(x) = \frac{\sin^2(x)}{x^2}$. The polynomial approximation $1 - \frac{x^2}{3}$ has an error bound $\abs{f(x) - 1 + \frac{x^2}{3}} \leq \epsilon_{\sinc}$ for $|x| \leq \sqrt[3]{6 \epsilon_{\sinc}}$. Further, the constant approximation $f(x) = 1$ has error $|f(x) - 1| \leq \epsilon_{\sinc}$ if $|x| \leq \sqrt{2 \epsilon_{\sinc}}$. Further, if $x = \Delta t$ with $\Delta \geq \Delta_{\min{}}$, we note that $t \geq (\Delta_{\min} \sqrt{\epsilon_{\sinc} } )^{-1}$ suffices for $f(\Delta t) \leq \epsilon_{\sinc}$.
\end{lemma}
\begin{proof}
    We use the form $\frac{\sin(x)}{x} = \int_0^1 \cos(sx) ds$. We compute the first 3 derivatives using straightforward algebra
    \begin{align}
    ...
    \begin{align}
        f(x) &= \frac{\sin^2(x)}{x^2} \bigg|_{x = 0} + x \frac{df}{dx}\bigg|_{x = 0} + \frac{x^2}{2!} \frac{d^2f}{dx^2}\bigg|_{x=0} + \frac{x^3}{3!} \frac{d^3f}{dx^3}\bigg|_{x = c} \\
        f(x) &= 1 - \frac{x^2}{3} + \frac{x^3}{3!} \frac{d^3f}{dx^3}\bigg|_{x = c} \\
        \abs{f(x) - 1 + \frac{x^2}{3}} & = \frac{|x|^3}{6} \abs{\frac{d^3f}{dx^3}\bigg|_{x=c}} \\
        &\leq \frac{|x|^3}{6}.
    \end{align}
    Setting $|x|^3/6 \leq \epsilon$ yields the statement.
...
\section{Single Qubit Environment}
...
\end{align}
    ...
    The other approach one could take is imagining cycling $\gamma$ through a series of exponentially decaying strengths. In this case we would choose some upper bound on the eigenvalue gaps, say $\Delta_{\max}$, and let $\gamma(k) = \Delta_{\min} + (\Delta_{\max} - \Delta_{\min}) \frac{2^{k} - 2^{-k}}{2^{k} + 2^{-k}}$. At $k = 0$ we have $\gamma(0) = \Delta_{\min}$ and as $k \to \infty$ we have $\gamma(k) \to \Delta_{\max}$. A simpler approach might just be $\gamma(k) = \Delta_{\max} 2^{-k}$. The dumbest approach might be $\gamma(k) = \Delta_{\min} + (\Delta_{\max} - \Delta_{\min}) \frac{k}{n} \mod n$. I also think the dumbest approach might be the best bet. 
