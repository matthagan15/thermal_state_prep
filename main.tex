...
\section{Boltzmann Taylor Series}
There exists a $\beta_{\star}$ such that
\begin{align}
    \frac{e^{-\beta \lambda_S(i)}}{\sum_j e^{-\beta \lambda_S(j)}} = \frac{e^{-\beta_E \lambda_S(i)}}{\sum_j e^{-\beta_E \lambda_S(j)}} + (\beta_E - \beta) \frac{e^{-\beta_{\star} \lambda_S(i)}}{\sum_j e^{-\beta_{\star} \lambda_S(j)}} \parens{\trace{H_S \frac{e^{-\beta_{\star} H}}{\trace{e^{-\beta_{\star}H_S}}}} - \lambda_S(i)}
\end{align}
...
Going to investigate a Taylor's series about $\beta_E$ for the Boltzmann coefficient of the ground state energy
\begin{align}
    \frac{e^{-\beta \lambda_S(0)}}{\partfun_S(\beta)} = \frac{e^{-\beta_E \lambda_S(0)}}{\partfun_S(\beta_E )} + (\beta - \beta_E) \frac{d}{d\beta} \frac{e^{-\beta \lambda_S(0)}}{\partfun_S(\beta)} \bigg|_{\beta \to \beta_E} + R_2(\beta).
\end{align}
Actually I think the first order term might be sufficient
\begin{align}
    \frac{e^{-\beta \lambda_S(0)}}{\partfun_S(\beta)} = \frac{e^{-\beta_E \lambda_S(0)}}{\partfun_S(\beta)} + R_0(\beta),
\end{align}
where we use the integral form of the $R_0$ remainder term as
\begin{align}
    R_0(\beta) = \int_{\beta}^{\beta_E} (x - \beta) \frac{d}{d\beta} \frac{e^{-\beta \lambda_S(0)}}{\partfun_S(\beta)}\bigg|_{\beta = x} dx .
\end{align}

Now we have to compute the derivatives. Up first is the derivative of the partition function
\begin{align}
    \frac{d}{d\beta} \partfun_S(\beta) &= \frac{d}{d\beta} \sum_i e^{-\beta \lambda_S(i)} \\
    &= \sum_i -\lambda_S(i) e^{-\beta \lambda_S(i)} \\
    &= -\trace{H e^{-\beta H}}.
\end{align}
Up second is the first derivative
\begin{align}
    \frac{d}{d\beta} \frac{e^{-\beta \lambda_S(0)}}{\partfun_S(\beta)} &= -\lambda_S(0) \frac{e^{-\beta \lambda_S(0)}}{\partfun_S(\beta)} - \frac{e^{-\beta \lambda_S(0)}}{\partfun_S(\beta)^2} \frac{d}{d\beta} \partfun_S(\beta) \\
    &= \frac{e^{-\beta \lambda_S(0)}}{\partfun_S(\beta)}\parens{\trace{H \frac{e^{-\beta H}}{\partfun_S(\beta)}} - \lambda_S(0)}.
\end{align}
We note that this can be extended to any Boltzmann coefficient for eigenvalue $i$ straightforwardly. Now we can plug this in to the first order remainder
\begin{align}
    R_0(\beta) = \int_{\beta}^{\beta_E} (x - \beta) \frac{e^{-x \lambda_S(0)}}{\partfun_S(x)}\parens{\trace{H \frac{e^{- x H}}{\partfun_S(x)}} - \lambda_S(0)} dx.
\end{align}
We note that this number is manifestly positive, as $\trace{H \frac{e^{-x H}}{\partfun_S(x)}} \geq \lambda_S(0)$ for all $x$. 

Now we note that we want to compute a lower bound on the difference:
\begin{align}
    ...
    &= \abs{R_0(\beta)},
\end{align}
where the last expression holds assuming that $\beta < \beta_E$.

As $x - \beta$ is positive for $x \geq \beta$, $\frac{e^{-x \lambda_S(0)}}{\partfun_S(x)} \geq 0$, and $\trace{H \frac{e^{-x H}}{\partfun_S(x)}} - \lambda_S(0) \geq 0$ we have to compute lower bounds for each of these expressions.

First up is 
\begin{align}
    \trace{H_S \frac{e^{-x H_S}}{\partfun_S(x)}} - \lambda_S(0) &= \trace{H_S \frac{e^{-x H_S}}{\partfun_S(x)}} - \lambda_S(0) \trace{\frac{e^{-x H_S}}{\partfun_S(x)}} \\
    &= \trace{\parens{H_S - \lambda_S(0) \identity} \frac{e^{-x H_S}}{\partfun_S(x)}} \\
    &= \sum_k (\lambda_S(k) - \lambda_S(0)) \frac{e^{-x \lambda_S(k)}}{\partfun_S(x)} \\
    &\geq \Delta_{\min} \sum_{k > 0} \frac{e{-x \lambda_S(k)}}{\partfun_S(x)} \\
    &= \Delta_{\min} \parens{1 - \frac{e^{-x \lambda_S(0)}}{\partfun_S(x)}}.
\end{align}
For the Boltzmann coefficients we will first try the simple bounds 
\begin{equation}
    \beta \leq x \leq \beta_E \implies \frac{e^{-\beta \lambda_S(0)}}{\partfun_S(\beta)} \leq \frac{e^{-x \lambda_S(0)}}{\partfun_S(x)} \leq \frac{e^{-\beta_E \lambda_S(0)}}{\partfun_S(\beta_E)}.
\end{equation}
This simplifies the remainder as
\begin{align}
    R_0(\beta) &\geq \Delta_{\min} \int_{\beta}^{\beta_E} (x - \beta) \frac{e^{-x \lambda_S(0)}}{\partfun_S(x)} \parens{1 - \frac{e^{-x \lambda_S(x)}}{\partfun_S(x)}} dx. \\
    ...
    &= \Delta_{\min} \frac{e^{-\beta \lambda_S(0)}}{\partfun_S(\beta)} \parens{1 - \frac{e^{-\beta_E \lambda_S(0)}}{\partfun_S(\beta_E)}} \frac{\delta^2}{2},
\end{align}
where $\delta = \beta_E - \beta$. Now we use the dumb bound that $\frac{e^{-\beta \lambda_S(0)}}{\partfun_S(\beta)} \geq \frac{1}{\dim}$ to get
\begin{align}
    R_0(\beta) \geq \frac{\Delta_{\min} \delta^2}{2 \dim_S} \parens{1 - \frac{e^{-\beta_E \lambda_S(0)}}{\partfun_S(\beta_E)}}.
\end{align}

The last bound we need is $1 - \frac{e^{-\beta_E \lambda_S(0)}}{\partfun_S(\beta_E)} \geq \epsilon_{\rho}$. This will be done through a bound on the partition function, seen below
\begin{align}
    \partfun_S(\beta_E) &=  \sum_k e^{-\beta_E \lambda_S(k)} \\
    &= e^{-\beta_E \lambda_S(0)} \parens{1 + \sum_{k > 0} e^{-\beta_E (\lambda_S(k) - \lambda_S(0))}}.
\end{align}
Rearranging yields
\begin{align}
    \frac{e^{-\beta_E \lambda_S(0)}}{\partfun_S(\beta_E)} &= \frac{1}{1 + \sum_{k > 0} e^{-\beta_E (\lambda_S(k) - \lambda_S(0))}}.
\end{align}
As we would like to upper bound this, we note that $\lambda_S(k) - \lambda_S(0) \geq \Delta_{\min}$, which yields
\begin{align}
    -\beta_E (\lambda_S(k) - \lambda_S(0)) &\geq - \beta_E \Delta_{\min} \\
    \iff \sum_{k > 0} e^{-\beta_E (\lambda_S(k) - \lambda_S(0)} &\geq \sum_{k > 0} e^{- \beta_E \Delta_{\min}} \\
    \iff \frac{1}{1 + \sum_{k > 0} e^{-\beta_E (\lambda_S(k) - \lambda_S(0))}} &\leq \frac{1}{1 + e^{- \beta_E \Delta_{\min}} (\dim_S - 1)}.
\end{align}
Through straightforward algebra, it can be seen that requiring 
\begin{equation}
    \beta_E \leq \frac{1}{\Delta_{\min}} \parens{\ln (\dim_S - 1) + \ln \parens{1 + \frac{1}{\epsilon_{\rho}}}} \label{eq:beta_e_upper_bound_thermal_diffs}
\end{equation}
is sufficient to guarantee that $1 - \frac{e^{-\beta_E \lambda_S(0)}}{\partfun_S(\beta_E)} \geq \epsilon_{\rho}$.

We can finally then declare that if $\beta_E$ satisfies the bound in Eq. \eqref{eq:beta_e_upper_bound_thermal_diffs} then we have
$$\norm{\rho_S(\beta_E) - \rho_S(\beta)}_1 \geq R_0(\beta) \geq \frac{\Delta_{\min} \delta^2 \epsilon_{\rho}}{2 \dim_S}.$$
...
\section{Possibly useful Lemmas}
...
\begin{proof}
    ...
    \begin{align}
        ...
        &\leq 2 \dim_S \norm{H} \delta,
    \end{align}
    which yields the implication in the lemma statement straightforwardly.\todo{Need to try and eliminate the factor of $\dim_S$ in the above lemma.}
\end{proof}
...
Goal is to bound the expectation over the perfect knowledge distribution for $\gamma$ of the second order correction $T_{\gamma}^{(2)}(\rho_S(\beta))$. Letting $p(i) = \frac{e^{-\beta \lambda_S(i)}}{\partfun_S(\beta)}$ This looks like
\begin{align}
    T_{\gamma}^{(2)} (\rho_S(\beta)) &= \sum_{i,j,k,l} \ketbra{k}{k} p(i) \frac{e^{-\beta_E \lambda_E(j)}}{\partfun_E(\beta_E)} \tau(i,j | k,l) \\
    &= \frac{\alpha^2 t^2}{\dim + 1} \sum_{k}\sum_{i \neq k} \sum_{j,l} \parens{p(i) \frac{e^{-\beta_E \lambda_E(j)}}{\partfun_E(\beta_E)} - p(k) \frac{e^{-\beta_E \lambda_E(l)}}{\partfun_E(\beta_E)}} \sinc^2((\Delta_S(i,k) + \Delta_E(j,l)) t/2) \ketbra{k}{k}.
\end{align}
We now make the dependence on $\gamma$ explicit by computing the sum over $j,l$. This is tedious, but given as
\begin{align}
    T^{(2)}_{\gamma}(\rho_S(\beta)) &= \frac{\alpha^2 t^2}{\dim + 1} \sum_{k} \sum_{i \neq k} \ketbra{k}{k} \frac{1}{1 + e^{-\beta_E \gamma}}\bigg[ (p(i) - p(k)) \sinc^2(\Delta_S(i,k) t/ 2) \nonumber \\
    &\quad + (p(i) - e^{-\beta_E \gamma} p(k))\sinc^2((\Delta_S(i,k) - \gamma)t/2) \nonumber \\
    &\quad + (e^{-\beta_E \gamma} p(i) - p(k)) \sinc^2((\Delta_S(i,k) + \gamma)t/2)  \nonumber \\
    &\quad + e^{-\beta_E \gamma} (p(i) - p(k)) \sinc^2(\Delta_S(i,k) t/2) \bigg] \label{tmp:big_second_order_sum}
\end{align}

Since our Hamiltonian is well-separated and we are sampling from the perfect knowledge distribution for each $i \neq k$ we will choose the one and only one $\gamma = \Delta_S(i,k)$ and all other $\gamma$'s will yield small transitions. For this reason, we now investigate the average of a single $(i,k)$ term over $\gamma$. This looks like:
\begin{align}
    \mathbb{E}_{\gamma} X(\gamma) = \sum_{(a,b) : a > b} \frac{2}{\dim_S (\dim_S - 1)} X(\Delta_S(a,b)).
\end{align}
We now evaluate a single summand of the $i,k$ sum in Eq. \eqref{tmp:big_second_order_sum}, but we do so term by term.
First we note that the terms where $\gamma$ is not in the $\sinc$ argument are trivially bounded from above and below:
\begin{align}
    \frac{1}{1 + e^{-\beta_E \gamma}} (p(i) - p(k)) \sinc^2(\Delta_S(i,k) t/2) &\leq \sinc^2(\Delta_S(i,k)t/2)\\
    &\leq \epsilon_{\sinc}
\end{align}
\begin{align}
    \frac{1}{1 + e^{-\beta_E \gamma}} (p(i) - p(k)) \sinc^2(\Delta_S(i,k) t/2) &\geq - \sinc^2(\Delta_S(i,k)t/2) \\
    &\geq - \epsilon_{\sinc}
\end{align}
which implies 
\begin{equation}
    -\epsilon_{\sinc} \leq \mathbb{E}_{\gamma} \frac{1}{1 + e^{-\beta_E \gamma}} (p(i) - p(k)) \sinc^2(\Delta_S(i,k) t/2) \leq \epsilon_{\sinc}.
\end{equation}
The exact same arguments imply
\begin{equation}
    -\epsilon_{\sinc} \leq \mathbb{E}_{\gamma} \frac{e^{-\beta_E \gamma}}{1 + e^{-\beta_E \gamma}} (p(i) - p(k)) \sinc^2(\Delta_S(i,k) t/2) \leq \epsilon_{\sinc}.
\end{equation}
Now we assume $i < k \implies \lambda_S(i) < \lambda_S(k) \implies \Delta_S(i,k) < 0$. Note that this implies that for a thermal state $p(i) \geq p(k)$. We can then reduce one of the remaining terms 
\begin{align}
    &\frac{1}{1 + e^{-\beta_E \gamma}}(p(i) - e^{-\beta_E \gamma} p(k))\sinc^2((\Delta_S(i,k) - \gamma)t/2) \\
    &=\frac{1}{1 + e^{-\beta_E \gamma}}(p(i) - e^{-\beta_E \gamma} p(k))\sinc^2((\Delta_S(k,i) + \gamma)t/2) \\
    &\leq \sinc^2((\Delta_S(k,i) + \gamma)t/2) \\
    &\leq \epsilon_{\sinc}.
\end{align}
Similar arguments work to show that this term is greater than $-\epsilon_{\sinc}$. We now turn to the last term, the one interesting term. We also will simplify the probability terms as follows
\begin{align}
    \frac{e^{-\beta_E \gamma}}{1 + e^{-\beta_E \gamma}}p(i) - \frac{1}{1 + e^{-\beta_E \gamma}}p(k) 
\end{align}
\begin{align}
    &\mathbb{E}_{\gamma} \frac{e^{-\beta_E \gamma} p(i) - p(k)}{1 + e^{-\beta_E \gamma}} \sinc^2((\Delta_S(i,k) + \gamma)t/2) \\
    &= \parens{\frac{2}{\dim_S(\dim_S - 1)}}\sum_{(a,b): a > b, (a,b) \neq (k,i)} \frac{e^{-\beta_E \Delta_S(b,a)} p(i) - p(k)}{1 + e^{-\beta_E \Delta_S(b,a)}} \sinc^2((\Delta_S(i,k) + \Delta_S(b,a))t/2) \nonumber \\
    &\quad + \frac{2}{\dim_S (\dim_S - 1)}\frac{e^{-\beta_E \Delta_S(k,i)} p(i) - p(k)}{1 + e^{-\beta_E \Delta_S(k,i)}} \sinc^2((\Delta_S(i,k) + \Delta_S(k,i)t/2).
\end{align} 
Again we bound the non-contributing indices straightforwardly
\begin{align}
    &\parens{\frac{2}{\dim_S(\dim_S - 1)}}\sum_{(a,b): a > b, (a,b) \neq (k,i)} \frac{1}{1 + e^{-\beta_E \Delta_S(b,a)}}(e^{-\beta_E \Delta_S(b,a)} p(i) - p(k)) \sinc^2((\Delta_S(i,k) + \Delta_S(b,a))t/2) \\
    & \leq \frac{2}{\dim_S (\dim_S - 1)} \sum_{(a,b): a > b, (a,b) \neq (k,i)} \epsilon_{\sinc} \\
    &\leq \epsilon_{\sinc},
\end{align}
and similarly $-\epsilon_{\sinc}$ forms a lower bound on this term. The remaining term is characterized by the function $\frac{e^{-\beta_E \Delta_S(k,i)} p(i) - p(k)}{1 + e^{-\beta_E \Delta_S(k,i)}}$.

Putting it all together, for $i < k$ we have 
\begin{align}
    -4 \epsilon_{\sinc} \leq \mathbb{E}_{\gamma} \sum_{j,l} (p(i) q(j) - p(k) q(l)) \tau(i,j | k,l) - \frac{2}{\dim_S(\dim_S - 1)} \frac{\alpha^2 t^2}{\dim + 1} \frac{e^{-\beta_E \Delta_S(k,i)} p(i) - p(k)}{1 + e^{-\beta_E \Delta_S(k,i)}} \leq 4 \epsilon_{\sinc}.
    \end{align}
Repeating the same process but with $i > k$, and therefore $\Delta_S(i,k) \geq 0$, we get
\begin{align}
    -4 \epsilon_{\sinc} \leq \mathbb{E}_{\gamma} \sum_{j,l} (p(i) q(j) - p(k) q(l)) \tau(i,j| k,l) - \frac{2}{\dim_S(\dim_S - 1)} \frac{\alpha^2 t^2}{\dim + 1} \frac{p(i) - e^{-\beta_E \Delta_S(i,k)} p(k)}{1 + e^{-\beta_E \Delta_S(i,k)}} \leq 4 \epsilon_{\sinc}
\end{align}
This then gives the lemma
\begin{align}
    \bra{k} \mathbb{E}_{\gamma} T_{\gamma}^{(2)}(\rho_S(\beta))\ket{k} 
\end{align}


So if we want to look at the single shot capture radius
\begin{align}
    \norm{\rho_S(\beta_E) - \mathbb{E}_{\gamma} \Phi_{\gamma}(\rho_S(\beta))}_1 &\leq \norm{\rho_S(\beta_E) - \rho_S(\beta) - \mathbb{E}_{\gamma}T^{(2)}(\rho_S(\beta)}_1 + \norm{R_{\Phi}}_1 \\
    &= \sum_k \abs{p_E(k) - p(k) - \mathbb{E}_{\gamma} \sum_{i \neq k} \sum_{j,l} p(i) q(j) \tau(i,j|k,l)} + \norm{R_{\Phi}}_1.
\end{align}
We now can zoom in on the sum over $i$:
\begin{align}
    \sum_{i < k} \mathbb{E}_{\gamma}p(i) q(j) \tau(i,j| k,l) + \sum_{i > k} \mathbb{E}_{\gamma}p(i) q(j) \tau(i,j| k,l)
\end{align}
\begin{align}
    &\abs{p_E(k) - p(k) - \sum_{i \neq k} \mathbb{E}_{\gamma} p(i) q(j) \tau(i,j|k,l)} \nonumber \\
    &\leq \abs{p_E(k) - p(k) - \frac{2}{\dim_S (\dim_S - 1)} \frac{\alpha^2 t^2}{\dim + 1}\parens{ \sum_{i < k} \frac{p(i) - e^{-\beta_E \Delta_S(k,i)} p(k)}{ 1 + e^{-\beta_E \Delta_S(k,i)}} + \sum_{i > k} \frac{e^{-\beta_E \Delta_S(i, k)} p(i) - p(k)}{ 1 + e^{-\beta_E \Delta_S(i, k)}}} } \nonumber \\
    &+ 4 (\dim_S - 1) \epsilon_{\sinc} \\
    &= \bigg| p_E(k) - p(k) \parens{1 - \frac{2}{\dim_S(\dim_S - 1)} \frac{\alpha^2 t^2}{\dim + 1} \sum_{i \neq k} \frac{1}{1 + e^{-\beta_E \Delta_S(i,k)}}} \nonumber \\
    &\quad \quad - \frac{2}{\dim_S (\dim_S - 1)} \frac{\alpha^2 t^2}{\dim + 1} \sum_{i \neq k} \frac{p(i)}{1 + e^{-\beta_E \Delta_S(i, k)}} \bigg| \nonumber \\
    &+ 4 (\dim_S - 1) \epsilon_{\sinc} \\
\end{align}
