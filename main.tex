...
\begin{theorem}[Approximate Fixed Points]
    ...
    \begin{equation}
        \alpha t \leq \sqrt{2} \epsilon_{fix}^{1/4} \sqrt{1 + \frac{1}{\dim}} \implies \norm{\rho_S(\beta_E) - \Phi(\rho_S(\beta_E))}_1 \leq \epsilon_{fix}.
    \end{equation}
    Further, if $\gamma$ is an energy that causes transitions (todo: make more precise) then we have that the following temperatures are not fixed
    \begin{align}
        \beta_E - \beta \geq \delta_{LB} \implies \norm{\rho_S(\beta) - \Phi^{(2)}(\rho_S(\beta))} \geq \epsilon_{fix}.
    \end{align}
\end{theorem}
\begin{proof}
    We start out with the expansion of the channel
    \begin{align}
        \norm{\rho_S(\beta_E) - \Phi(\rho_S(\beta_E))}_1 &= \norm{\Phi^{(2)}(\rho_S(\beta_E)) + R_{\Phi}(\rho_S(\beta_E))}_1 \\
        &\leq \norm{\Phi^{(2)}(\rho_S(\beta_E))}_1 + \norm{R_{\Phi}(\rho_S(\beta_E))}_1 \\
        &\leq \norm{\Phi^{(2)}(\rho_S(\beta_E))}_1 + \dim \norm{R_{\Phi}} \\
        &\leq \norm{\Phi^{(2)}(\rho_S(\beta_E))}_1 + \dim \epsilon_R.
    \end{align}
    We will use up half of our error budget for this, $\epsilon_R = \epsilon_{fix} / (2 \dim)$, leading us to require that $\norm{\Phi^{(2)}(\rho_S(\beta_E))}_1 \leq \epsilon_{fix} / 2$. 

    Plugging in the second order correction for the channel gives us
    \begin{align}
        \norm{\Phi^{(2)}(\rho_S(\beta_E)}_1 &= \sum_k \abs{\frac{\alpha^2 t^2}{2}}
    \end{align}
\end{proof}

Need to update this theorem: Given a well separated Hamiltonian, for all values of $\gamma$ then the thermal state is an approximate fixed point.
\begin{theorem} \label{thm:approximate_fixed_point}
...
\begin{lemma}
    Assume that $H_S$ is a $(\Delta_{\min}, \Delta_{\sinc}, \epsilon_{\sinc}$)-separated Hamiltonian satisfying Def. \ref{def:separated_hamiltonians}. Let $(u,l)$ denote the unique index pair such that $|\Delta_S(u,l) - \gamma| \leq \Delta_{\sinc}$ and for all $(i,j) \neq (u,l)$ we have $|\Delta_S(i,j) - \gamma| \geq \Delta_{\min}$. We let $u$ denote the higher energy state, i.e $\lambda_S(l) < \lambda_S(u)$. Using $\delta \coloneqq \beta_E - \beta$, we show that the assumptions on $H$ and $\gamma$ along with the requirement $\delta \geq \beta_E \frac{\Delta_{\min}}{\Delta_S(u,l)} + \frac{1}{\Delta_S(u,l)} \ln \parens{\frac{1}{(2\dim + 1) \epsilon_{\sinc}}}$ imply the following bound
    % \begin{align}
    %     -2 \alpha^2 t^2 \epsilon_{\sinc} \parens{1 + \frac{1}{\dim + 1} + \frac{\epsilon_{exp}}{\dim + 1} }\leq \sum_{i, j, k} \frac{e^{-\beta \lambda_S(i)} e^{-\beta_E \lambda_E(j)}}{\partfun_S(\beta) \partfun_E(\beta_E)} \tau(i,j|l,k) - \frac{\alpha^2 t^2}{\dim + 1} \frac{e^{-\beta \lambda_S(u)}}{\partfun_S(\beta) \partfun_E(\beta_E)} \leq 2 \epsilon_{\sinc} \alpha^2 t^2
    % \end{align}
    \begin{align}
        \abs{\sum_{i, j, k} \frac{e^{-\beta \lambda_S(i)} e^{-\beta_E \lambda_E(j)}}{\partfun_S(\beta) \partfun_E(\beta_E)} \tau(i,j|l,k) - \frac{\alpha^2 t^2}{\dim + 1} \frac{e^{-\beta \lambda_S(u)}}{\partfun_S(\beta) \partfun_E(\beta_E)}} \leq 4 \epsilon_{\sinc} \alpha^2 t^2
    \end{align}
\end{lemma}
\begin{proof}
...
\begin{theorem}[Cooling Schedule]
    Suppose
\end{theorem}

\begin{claim}
    This is some bogus stuff
\end{claim}
\begin{proof}
