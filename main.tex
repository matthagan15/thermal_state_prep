...
\begin{proof}
    ...
    \end{align}
    Now we use Eq. \eqref{eq:phi_g_definition} to compute the derivatives
    \begin{align}
        \frac{\partial}{\partial \alpha} \Phi_G (\rho_S) &= \parens{\frac{\partial}{\partial \alpha} e^{+ i (H + \alpha G)t}} \rho_S e^{-i (H + \alpha G) t} + e^{+i (H + \alpha G)t} \rho_S \parens{\frac{\partial}{\partial \alpha} e^{- i (H + \alpha G)t}} \\
        &=  
    \end{align}
\end{proof}
...
For ease of writing, let $\rho_E \coloneqq \frac{e^{-\beta_E H_E}}{\partfun_E(\beta_E)}$ represent the resource state that the environment is prepared in. Letting $\rho_S$ denote the input system state, we use $\rho_T = \rho_S \otimes \rho_E$ to denote the input state of the combined system and environment. Using this, the first order term can be computed as
\begin{align}
    \frac{\partial}{\partial \alpha} \Phi_G(\rho_S) \bigg|_{\alpha = 0} &= \frac{\partial}{\partial \alpha} e^{+i(H + \alpha G) t} \rho_T e^{-i(H + \alpha G) t} \\
    &= \int_{0}^{1} e^{i s (H+\alpha G)t} (i t G) e^{i (1-s) (H+\alpha G)t} ds \rho_T e^{-i(H+\alpha G)t} \bigg|_{\alpha=0} \nonumber \\
    &~ + e^{i(H+\alpha G)t} \rho_T \int_{0}^1 e^{-i s (H+\alpha G) t} (- i t G) e^{-i (1-s) (H+\alpha G)t} ds \bigg|_{\alpha = 0} \label{eq:first_order_alpha_derivative}\\
    &= i t \int_0^1 e^{i s H t} G e^{-i s H t} ds e^{i H t} \rho_T e^{-i H t} - i t e^{+i H t} \rho_T \int_0^1 e^{-is H t} G e^{-i(1-s) Ht} ds \\
    &= i t \int_0^1 G(s t) ds \rho_T(t) - it \rho_T(t) \int_0^1 G(s t) ds \\
    &= i t \int_0^1 [G(s t), \rho_T(t)] ds,
\end{align}
where we utilize the Heisenberg picture $G(t) = e^{+i H t} G e^{-i H t}$, similarly for $\rho \otimes \rho_E$.
...
\begin{align}
    ...
    &= i t \partrace{env}{\int_0^1 \int [e^{i H s t} G e^{-i H s t}, \rho_T(t)] dG ds}.
\end{align}
From here we use the fact that partial traces, integrations, and our commutator is linear in one of the inputs to pull the averaging over $G$ into the commutator. Then we use the fact that any given $G$ can be written as $G = U_G D U_G^\dagger$ to yield
\begin{align}
    &= i t \partrace{env}{\int_0^1  [e^{i H s t} \int U_G D U_G^\dagger ~ dD ~ dU_G e^{-i H s t}, \rho_T(t)] ds} \\
    &= i t \partrace{env}{\int_0^1  [e^{i H s t} \int U_G \parens{\int D dD} U_G^\dagger ~ dU_G ~ e^{-i H s t}, \rho_T(t)] ds}.
\end{align}
...
Using substack
\begin{align}
    \frac{\partial^2}{\partial \alpha^2} \Phi_G(\rho) \bigg|_{\alpha = 0} =& \frac{\partial}{\partial \alpha} \parens{\int_{0}^{1} \underset{\substack{\downarrow \\ (A)}}{e^{i s (H+\alpha G)t}} (i t G) \underset{\substack{\downarrow \\ (B)}}{e^{i (1-s) (H+\alpha G)t}} ds \rho \underset{\substack{\downarrow \\ (C)}}{e^{-i(H+\alpha G)t}} } \bigg|_{\alpha=0} \nonumber \\
    &~ ~+\frac{\partial}{\partial \alpha} \parens{ \underset{\substack{\downarrow \\ (D)} }{e^{i(H+\alpha G)t}} \rho \int_{0}^1 \underset{\substack{\downarrow \\ (E)} }{e^{-i s (H+\alpha G) t} } (- i t G) \underset{\substack{\downarrow \\ (F)}}{e^{-i (1-s) (H+\alpha G)t}} ds } \bigg|_{\alpha = 0},
\end{align}
