...
Now we can perform the $\gamma$ expectation. For this we will integrate a uniformly chosen $\gamma$ from $\gamma_L$ to $\gamma_U$. We need to ensure that every value of $\Delta_S(i,j)$ has a window of $\pm \delta_{\min}$ around it so that each $\sinc$ integral will evaluate to the same amount. This means our limits have to be at least $\gamma_L = \min_{k \neq l} \Delta_S(k, l) - \delta_{\min}$ and $\gamma_U = \max_{k \neq l} \Delta_S(k,l) + \delta_{\min}$. \matt{Conjecture - $\gamma_L = 0$ and $\gamma_U = 2 \norm{H_S}$ should work.} For now, define $\gamma_\Delta = \gamma_U - \gamma_L = \max_{k \neq l} \Delta_S(k,l) - \min_{k \neq l} \Delta_S(k,l)$. This means that the integrations can be computed as
\begin{align}
    &\EE_\gamma \mathbf{I}[|\Delta_S(i,j) - \gamma| \le \delta_{\min}] \sinc^2 \left( \frac{(\Delta_S(i, j) - \gamma) t}{2} \right) \nonumber \\
    &= \int_{\gamma_L}^{\gamma_U} \mathbf{I}[|\Delta_S(i,j) - \gamma| \le \delta_{\min}] \sinc^2 \left( \frac{(\Delta_S(i, j) - \gamma) t}{2} \right) \frac{1}{\gamma_\Delta} d\gamma \\
    &= \frac{1}{\gamma_\Delta} \int_{\Delta_S(i,j) - \delta_{\min}}^{\Delta_S(i,j) + \delta_{\min}} \sinc^2 \left( \frac{(\Delta_S(i, j) - \gamma) t}{2} \right) d\gamma \\
    &= \frac{1}{\gamma_\Delta} \int_{-\delta_{\min}}^{\delta_{\min}} \sinc^2 \left( \frac{\widetilde{\gamma} t}{2} \right) d\widetilde{\gamma} \\
    &= \frac{2}{t(\gamma_U - \gamma_L)} \int_{-\delta_{\min} t/2}^{\delta_{\min} t/2} \sinc^2 (u) du 
\end{align}
where we label the last integral with $I_{\sinc}(t)$ which is defined as
\begin{equation}
    I_{\sinc}(t) \coloneqq  \int_{-\delta_{\min} t/2}^{\delta_{\min} t/2} \sinc^2 (u) du  . \label{eq:ground_state_prep_sinc_integral}
\end{equation}
For now we conjecture that this integral is $\widetilde{\Theta}(1)$ leaving the proof until later.
...
Now we should verify the behavior of $I_{\sinc}(t)$. By substituting our known value of $t$ from Eq. \eqref{eq:ground_state_prep_time} into definition in Eq. \eqref{eq:ground_state_prep_sinc_integral} we get
\begin{align}
    I_{\sinc}(t) &= \int_{-\delta_{\min} t/2}^{\delta_{\min} t/2} \sinc^2 (u) du \\
    &= \int_{0}^{\frac{\dim_S^2 (\gamma_U - \gamma_L)}{\epsilon \delta_{\min}}} \sinc^2 (u) du 
\end{align}
Now we know
\begin{align}
    I_{\sinc}(t) \leq \int_0^\infty \sinc^2(u) du \leq \frac{\pi}{2}.
\end{align}


I should turn this into some kind of bound. do I need an upper or lower on $I_{\sinc}$. I need a lower bound on $L > L_{LB} > \frac{1}{\lambda_{\star}}$. this corresponds to a lower bound on $\lambda_{\star}$ of $1/L_{LB}$

\end{proof}
\matt{TODO need to bound the $I_{\sinc}(t)$ contribution, I'm very certain it is constant but it needs to be proven nonetheless.}


Now we show how perfect knowledge of the eigenvalue differences with the ground state can affect the runtime. We now choose a distribution of $\gamma$ that reflects this knowledge. Let 
\begin{equation}
    S_{\Delta} \coloneqq \set{\Delta_S(i, j) : i > j},
\end{equation}
and we sample from this set while reflecting the degeneracy of each eigenvalue difference. This reflects on $\gamma$ as 
\begin{equation}
    \prob{\gamma = \Delta_S(i,j)} = \frac{\eta(i,j)}{\dim_S(\dim_S - 1)},
\end{equation}
this can be thought of as randomly choosing a pair of eigenvectors and setting $\gamma$ to be their 
