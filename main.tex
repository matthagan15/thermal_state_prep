...
\begin{equation}
    \Phi(\rho) := \int \partrace{\hilb_E}{e^{+i(H + \alpha G)t} \rho \otimes \rho_E e^{-i(H + \alpha G) t}} dG.
\end{equation}
...
The structure of the proof will work by noting that since $A$ is a Hermitian operator we can use the Jordan-Hahn decomposition to write $A = P - Q$, where $P$ is the part of the matrix resulting from the positive eigenvalues of $A$ and $Q$ the negative ones (which we then flip the sign). This yields the following
\begin{align}
    \bra{k}P\ket{k} &\coloneqq \begin{cases}
        \bra{k} (\rho_S(\beta_E) - \rho_S(\beta)) \ket{k} & \bra{k} \rho_S(\beta_E) \ket{k} \geq \bra{k} \rho_S(\beta) \ket{k} \\
        0 & else
    \end{cases} \\
    \bra{k}Q\ket{k} &\coloneqq \begin{cases}
        (-1)\bra{k} (\rho_S(\beta_E) - \rho_S(\beta)) \ket{k} & \bra{k} \rho_S(\beta_E) \ket{k} < \bra{k} \rho_S(\beta) \ket{k} \\
        0 & else
    \end{cases},
\end{align}
which yields $A = P - Q$ for both $P$ and $Q$ being PSD. This now allows us to break up the trace of $AB$ as
$\trace{AB} = \trace{PB} - \trace{QB}$. Since $P$ and $Q$ are both PSD we can then use upper and lower bounds on $B$ to produce a bound on $\trace{AB}$. Note if we can show that $2 A(k) \geq B(k) + \epsilon / \dim_S$ then we win.

To help with our analysis we will make use of a few results. The first being that by restricting ourselves to thermal state inputs  we can argue the following about $P$ and $Q$. We first note that our indices are sorted by eigenvalue of $H_S$, so $i \leq j \implies \lambda_S(i) \leq \lambda_S(j)$. As the exponential function $x \mapsto e^x$ is monotonic we have that 
$$k_P \in \im(P), k_Q \in \im(Q) \implies \frac{e^{-\beta \lambda_S(k_P)}}{\partfun_S(\beta)} \geq \frac{e^{-\beta \lambda_S(k_Q)}}{\partfun_S(\beta)}.$$ We use this sorting to create the (ordered) set of indices as before,
\begin{equation}
...
\begin{align}
    B(k_1) &\leq  \parens{\sum_{i \neq k_1, k_2} \sum_{j,l} (a(i) b(j) - a(k_1) b(l)) \tau(i,j|k_1, l) + 3 \epsilon_{\sinc} + (a(k_2) b(0) - a(k_1) b(1)) \tau(k_2, 0 | k_1, 1)} \\
    &\leq \parens{\sum_{i \neq k_1, k_2} \sum_{j,l} \epsilon_{\sinc} + 3 \epsilon_{\sinc} + (a(k_2) b(0) - a(k_1) b(1)) \tau(k_2, 0 | k_1, 1)} \\
    &= (4 \dim_S - 5) \epsilon_{\sinc} + (a(k_2) b(0) - a(k_1) b(1)) \tau(k_2, 0 | k_1, 1) \\
    &\leq 4 \dim_S \epsilon_{\sinc} + \tau(k_2, 0| k_1, 1)
\end{align}
and similarly we compute
\begin{align}
    B(k_2) &\leq  \parens{\sum_{i \neq k_1, k_2} \sum_{j,l} (a(i) b(j) - a(k_1) b(l)) \tau(i,j|k_2, l) + 3 \epsilon_{\sinc} + (a(k_1) b(1) - a(k_2) b(0)) \tau(k_1, 1 | k_2, 0)} \\
    &\leq \parens{\sum_{i \neq k_1, k_2} \sum_{j,l} \epsilon_{\sinc} + 3 \epsilon_{\sinc} + (a(k_1) b(1) - a(k_2) b(0)) \tau(k_1, 1 | k_2, 0)} \\
    &= (4 \dim_S - 5) \epsilon_{\sinc} + (a(k_1) b(1) - a(k_2) b(0)) \tau(k_1, 1 | k_2, 0) \\
    &\leq 4 \dim_S \epsilon_{\sinc} + \tau(k_1, 1| k_2, 0).
\end{align}
Given that $\tau$ is symmetric about input and output, we have $B(k_1) - (4 \dim_S - 5) \epsilon_{\sinc} = - (B(k_2) - (4\dim_S - 5)\epsilon_{\sinc})$. This allows us to compute the sum
\begin{align}
...
\begin{align}
    ...
    &\leq 16 |T_{\gamma}| (\dim_S - 1)^2 \epsilon_{\sinc}^2 + \sum_{(k_1, k_2) \in S_{\gamma}} B(k_1)^2 + B(k_2)^2 \\
    &\leq 16 |T_{\gamma}| (\dim_S - 1)^2 \epsilon_{\sinc}^2 \nonumber \\
    &+ \sum_{(k_1, k_2) \in S_{\gamma}} \bigg[ \parens{\sum_{i \neq k_1, k_2} \sum_{j,l} (a(i) b(j) - a(k_1) b(l)) \tau(i,j|k_1,l) + \sum_{j,l} (a(k_2) b(j) - a(k_1) b(l)) \tau(k_2, j| k_1, l)}^2 \nonumber \\
    &+ \parens{\sum_{i \neq k_1, k_2} \sum_{j,l} (a(i) b(j) - a(k_2) b(l)) \tau(i,j|k_2,l) + \sum_{j,l} (a(k_1) b(j) - a(k_2) b(l)) \tau(k_1, j| k_2, l)}^2 \bigg] 
\end{align}
...
\begin{align}
    \abs{S_1} &\leq \sum_{i \neq } \sum_{i \neq k_1, k_2} \sum_{j,l} |a(i) b(j) - a(k_1) b(l)| |\tau(i, j|k_1, l)| \\
    &\leq \epsilon_{\sinc} \sum_{i \neq } \sum_{i \neq k_1, k_2} \sum_{j,l} \\
    &\leq 4 (\dim_S - 2) \epsilon_{\sinc} \\
    \abs{S_2} &\leq \sum_{(j,l) \neq (0,1)} |a(k_1) b(j) - a(k_2) b(j)| |\tau(k_2, j | k_1, l)| \\
    &\leq \epsilon_{\sinc} \sum_{(j,l) \neq (0,1)} \\
    &\leq 3 \epsilon_{sinc}
\end{align}
...
Now our goal is to upper bound $-2\trace{AB}$. Similarly to before, we start off by assuming that $k \in T_{\gamma}$:
\begin{align}
    -2 B(k) &= -2 \sum_{i \neq k} \sum_{j,l} (a(i) b(j) - a(k) b(l)) \tau(i,j| k,l) \\
    &\leq 8 \dim_S \epsilon_{\sinc}.
\end{align}
This allows us to write
\begin{align}
    -2 \sum_{k \in T_{\gamma}} A(k) B(k) &\leq 8 \dim_S \epsilon_{\sinc} \sum_{k \in T_{\gamma}} A(k) \\
    &\leq 8 \dim_S |T_{\gamma}| \epsilon_{\sinc}
\end{align}
Now let $(k_1, k_2) \in S_{\gamma}$
\begin{align}
    -2 B(k_1) &= -2 \sum_{i \neq k_1, k_2} \sum_{j,l}(a(i) b(j) - a(k_1) b(l)) \tau(i, j | k_1, l) - 2 \sum_{j,l} (a(k_2) b(j) - a(k_1) b(l)) \tau(k_2, j | k_1, l) \\
    &\leq 8 (\dim_S - 2) \epsilon_{\sinc} + 6 \epsilon_{\sinc} - 2 (a(k_2) b(0) - a(k_1) b(1)) \tau(k_1, 1 | k_2, 0) \\
    &\leq 8 \dim_S \epsilon_{\sinc}  - 2 (a(k_2) b(0) - a(k_1) b(1)) \tau(k_1, 1 | k_2, 0).
\end{align}
Note that the above bound can be reproduced even if multiplied by a negative number. For example, suppose $A(k) \leq 0$:
\begin{align}
    -2 B(k) &= -2 \sum_{i \neq k} \sum_{j,l} (a(i) b(j) - a(k) b(l)) \tau(i,j|k,l) \\
    &= \sum_{i \neq k} \sum_{j,l} -2 a(i) b(j)\tau(i,j|k,l) + 2 a(k) b(l) \tau(i,j|k,l) \\
    &\geq \sum_{i \neq k} \sum_{j,l} -2 \tau(i,j|k,l) \\
    &\geq -8 (\dim_S - 1) \epsilon_{\sinc}\\
    &\geq -8 \dim_S \epsilon_{\sinc}.
\end{align}
Now we can add the terms:
\begin{align}
    -2 A(k_1) B(k_1) - 2 A(k_2) B(k_2) &\leq 
\end{align}
Now we combine these to yield: 
\begin{align}
