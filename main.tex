...
\end{align}
Curiously, there are no requirements on the value of $t$. To complete this arguement, we now simplify the lower bound on $\epsilon$ by noting,
\begin{align}
...
\textbf{Warning: The below is incorrect due to a missed factor of $t^2$}
...
\begin{align}
    &(\alpha t)^4 \parens{2 t^4 r^2 \sum_{(k_1, k_2) \in S_{\gamma}} \frac{\sinc((\Delta_S(k_1, k_2) - \gamma) t/2)^4}{(\dim + 1)^2}} \nonumber \\
    &+ (\alpha t)^2 \parens{2 r \sum_{(k_1, k_2) \in S_{\gamma}} \frac{\sinc((\Delta_S(k_1, k_2) - \gamma) t/2)^2}{\dim + 1} (A(k_2) - A(k_1))} \nonumber \\
...
$$d \coloneqq 2 r \sum_{(k_1, k_2) \in S_{\gamma}} \frac{\sinc((\Delta_S(k_1, k_2) - \gamma) t/2)^2}{\dim + 1} (A(k_2) - A(k_1)).$$
Letting $x$ play the role of $(\alpha t)^2$ gives us a simple quadratic expression:
\begin{equation}
