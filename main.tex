...
Starting with safe parameter choices of $\Delta = \gamma = 1$, $\alpha = 0.01$, $t = 100.0$, and $\beta = 3.0$ we show how a harmonic oscillator truncated to the $4$ lowest energy levels thermalizes. Below is a plot for how this error behaves, we have taken only 8 samples
\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{numerics/data/sho_error_vs_l.png}
    \caption{Trace distance to target as a function of interaction number, simulation compared to Markov chain behavior. The Hilbert-Schmidt overlap with the ground state $\anglebrackets{\Phi^L(\rho(0)), \ketbra{0}{0}}$ is 0.950 at $L = 100$.}
