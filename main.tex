...
The first inequality is rather straightforward, using the bound that $\epsilon_{\sinc} \geq \frac{1}{\Delta_{\min}^2 t^2}$ and that $\epsilon_{\sinc} \leq \frac{1}{2\dim_S}$ implies $16 \dim_S^3 \epsilon_{\sinc}^2 \leq 8 \dim_S^2 \epsilon_{\sinc}$ to get a ``sandwich" of inequalities for $\epsilon_{\sinc}$. Then taking $t$ sufficiently large $t \geq \frac{4 \sqrt{2} \dim_S}{\Delta_{\min} \sqrt{\epsilon}}$ suffices to give an appropriate range for $\epsilon_{\sinc}$.
...
\begin{align}
    &\alpha^4 \parens{2 t^4 r^2 \sum_{(k_1, k_2) \in S_{\gamma}} \frac{\sinc((\Delta_S(k_1, k_2) - \gamma) t/2)^4}{(\dim + 1)^2}} \nonumber \\
    &+ \alpha^2 \parens{2 t^2 r \sum_{(k_1, k_2) \in S_{\gamma}} \frac{\sinc((\Delta_S(k_1, k_2) - \gamma) t/2)^2}{\dim + 1} + 2r (A(k_2) - A(k_1))} \nonumber \\
    &+ \alpha^0 \frac{3 \epsilon}{2} \leq 0
\end{align}
We first simplify the coefficient for $\alpha^4$ as 
\begin{align}
    2 t^4 r^2 \sum_{(k_1, k_2) \in S_{\gamma}} \frac{\sinc((\Delta_S(k_1, k_2) - \gamma) t/2)^4}{(\dim + 1)^2} &\leq 2 t^4 \frac{|S_{\gamma}|}{(\dim + 1)^2}.
\end{align}
For simplicity we denote the coefficient for $\alpha^2$ as
$$d \coloneqq 2 t^2 r \sum_{(k_1, k_2) \in S_{\gamma}} \frac{\sinc((\Delta_S(k_1, k_2) - \gamma) t/2)^2}{\dim + 1} + 2r (A(k_2) - A(k_1)).$$
Letting $x$ play the role of $\alpha^2$ gives us a simple quadratic expression:
\begin{equation}
    x^2 \frac{2 t^4 |S_{\gamma}|}{(\dim + 1)^2} + x d + \frac{3 \epsilon}{2} \leq 0.
\end{equation}
As $\alpha \geq 0$ we have $x \geq 0$ as well. This requires the roots of the quadratic to be both real and at least one of them must be positive. Denote the roots $x_{\pm}$, which can be seen as:
\begin{align}
    x_{\pm} &= \frac{-d(\dim + 1)^2}{4t^4 |S_{\gamma}|} \pm \frac{(\dim + 1)^2}{4t^4 |S_{\gamma}|} \sqrt{d^2 - \frac{12 t^4 |S_{\gamma}| \epsilon}{(\dim + 1)^2}} \\
    &= \frac{-d(\dim + 1)^2}{4t^4 |S_{\gamma}|} \parens{1 \mp \sqrt{1 - \frac{12 t^4 |S_{\gamma}| \epsilon}{(\dim + 1)^2 d^2}}}.
\end{align}
We want to guarantee that these roots are real, so we require 
\begin{equation}
    d^2 \geq \frac{12 t^4 |S_{\gamma}| \epsilon}{(\dim + 1)^2}.
\end{equation}
Note that inside the radical, $12 t^4 |S_{\gamma}| \epsilon / (\dim + 1)^2 d^2$ is always positive, implying that the radical is always going to be less than 1. If we would like the value of resulting value of $\alpha$ to be positive, we require that $d \leq 0$. 

If these two conditions are met, then we can set $\alpha^2$ to be the average of the two roots and the distance reduction claim is satisfied. Ideally we would like the lower root to be as close to 0 as possible, so that way we can simply reduce $\alpha$ to get our desired distance reduction. Regardless, our last objective still is to produce bounds on $d$ that satisfy the given inequalities, we therefore turn our attention to the sum in question.

\begin{align}
    d = 2 t^2 r \sum_{(k_1, k_2) \in S_{\gamma}} \frac{\sinc((\Delta_S(k_1, k_2) - \gamma) t/2)^2}{\dim + 1} + 2r (A(k_2) - A(k_1)),
\end{align}
where we remind the reader that $r = \bra{k_2} \rho_S \ket{k_2} \frac{e^{-\beta_E \lambda_E(0)}}{\partfun_E(\beta_E)} - \bra{k_1} \rho_s \ket{k_1} \frac{e^{-\beta_E \lambda_E(1)}}{\partfun_E(\beta_E)}$ and $A(k) = \frac{e^{-\beta_E \lambda_S(k)}}{\partfun_S(\beta_E)} - \bra{k} \rho_S \ket{k}$.
Given that we would like to bound these sums, we need to know when $r$ is positive or negative.
\begin{equation}
    r \geq 0 \implies \frac{\bra{k_2} \rho_S \ket{k_2}}{\bra{k_1} \rho_S \ket{k_1}} \geq e^{-\beta_E \gamma}.
\end{equation}
Given that we would like this process to replicate a thermalization/cooling procedure, where our system is in a thermal state and we are slowly reducing the temperature, we should require that the higher energy states $\bra{k_2} \rho_S \ket{k_2}$ have smaller probability than lower energy states $\bra{k_1} \rho_S \ket{k_1}$

Either way, we know that $-1 \leq r \leq 1$, so $r$ should not affect the asymptotics. Let us use the upper bound of 1 for now
$$d \leq 2t^2 \sum_{(k_1, k_2) \in S_{\gamma}} \frac{\sinc((\Delta_S(k_1, k_2) - \gamma) t/2)^2}{\dim + 1} + 2(A(k_2) - A(k_1)).$$
Similarly, we know that the $\sinc$ is upper bounded by 1, yielding
\begin{align}
     d &\leq \frac{2t^2 |S_{\gamma}|}{\dim + 1}  +2 \sum_{(k_1, k_2) \in S_{\gamma}} A(k_2) - A(k_1) \\
     &= \frac{2t^2 |S_{\gamma}|}{\dim + 1}  +2 \sum_{(k_1, k_2) \in S_{\gamma}} \frac{e^{-\beta_E \lambda_S(k_2)}}{\partfun_S(\beta_E)} - \bra{k_2} \rho_S \ket{k_2} - \frac{e^{-\beta_E \lambda_S(k_1)}}{\partfun_S(\beta_E)} + \bra{k_1} \rho_S \ket{k_1}.
\end{align}
Without further knowledge of the structure of $\rho_S$ we cannot produce any more simplifying bounds. This leads to the $d \leq 0$ bound yielding 
\begin{align}
    \sum_{(k_1, k_2) \in S_{\gamma}} \bra{k_1} \rho_S \ket{k_1} - \bra{k_2} \rho_S \ket{k_2} \leq \sum_{(k_1, k_2) \in S_{\gamma}}\frac{e^{-\beta_E \lambda_S(k_1)} - e^{-\beta_E \lambda_S (k_2)}}{\partfun_S(\beta_E)} - \frac{2 t^2 |S_{\gamma}|}{\dim + 1}.
\end{align}
This unfortunately does not seem possible, as $t \in \Omega(\dim_S) = \Omega(\dim)$. 

Given that the dominant term in $d$ is the $\bigo{t^2}$ term, in order for this sum to be negative we will have to require that $r \leq 0$. This boils down to the following
\begin{align}
    r &\leq 0 \\
    \bra{k_2} \rho_S \ket{k_2} \frac{1}{\partfun_E(\beta_E)} - \bra{k_1} \rho_S \ket{k_1} \frac{e^{-\beta_E \gamma}}{\partfun_E(\beta_E)} &\leq 0 \\
    \frac{\bra{k_2} \rho_S \ket{k_2}}{\bra{k_1} \rho_S \ket{k_1}} &\leq e^{-\beta_E \gamma}.
\end{align}
This bound is rather abstract, but boils down to requiring the high energy terms to having lower probability than the low energy terms. If we assume $\rho_S = \frac{e^{-\beta H_S}}{\partfun_S(\beta)}$, then this inequality can be expressed as $\beta_E \leq \beta \frac{\Delta_S}{\gamma}$. Since we are hoping to cool, this would imply $\beta \leq \beta_E$. The inequality we require indicates a lower bound on how much larger this temperature (or smaller $\beta$) can be. Expressed in terms of $\gamma$ we have $\gamma \leq \Delta_S \frac{\beta}{\beta_E}$.
So we assume that this holds for the provided $\rho_S, \beta_E,$ and $\gamma$.

Now in order to show negativity of $d$ we just have to find when the $t^2$ term dominates the $A$ contributions. We need to find the value of $t^2$ such that 
\begin{equation}
    t^2 \sum_{(k_1, k_2) \in S_{\gamma}} \frac{\sinc((\Delta_S(k_2, k_1) - \gamma)t/2)^2}{\dim + 1} \geq \sum_{(k_1, k_2) \in S_{\gamma}} (A(k_1) - A(k_2)). \label{eq:dominant_inequality_in_dist_reduction}
\end{equation}
Note that since we are summing over $S_{\gamma}$ we can assume that $\sinc^2 \geq \epsilon_{\sinc}$. We also can make the dumb bound that $\sum_{(k_1, k_2) \in S_{\gamma}} A(k_1) - A(k_2) \leq \sum_{(k_1, k_2) \in S_{\gamma}} A(k_1) \leq |S_{\gamma}|$. This gives us the implication that if $\frac{t^2 \epsilon_{\sinc}|S_{\gamma}|}{\dim + 1} \geq |S_{\gamma}|$ holds then the bound in \eqref{eq:dominant_inequality_in_dist_reduction} holds as well. This inequality is then sufficient to claim that $d \leq 0$.

We now need to verify the other claim, that $d^2 \geq \frac{12 t^4 |S_{\gamma}| \epsilon}{(\dim + 1)^2}$. As we know $d$ negative, we should aim to produce lower bounds on $d$. Since the sign of $d$ is carried by $r$ we only need to upper bound the summation.
\begin{align}
    L \coloneqq 2 \sum_{(k_1, k_2) \in S_{\gamma}} t^2 \frac{\sinc((\Delta_S(k_2, k_1) - \gamma)t/2)^2}{\dim + 1} + A(k_2) - A(k_1) & \geq 2 |S_{\gamma}| \parens{\frac{t^2 \epsilon_{\sinc}}{\dim + 1}  -2}.
\end{align}
Since $d = r * L$, we have $d^2 = r^2 L^2 \geq (-1)^2 L^2$. We now plug in the lower bound for $L^2$ to our desired inequality to get a bound on the allowed values of $\epsilon$
\begin{align}
    4 |S_{\gamma}|^2 \parens{\frac{t^2 \epsilon_{\sinc}}{\dim + 1} - 2}^2 &\geq 12\frac{t^4 |S_{\gamma}| \epsilon}{(\dim + 1)^2}
     \\
    \epsilon &\leq \frac{1}{3} |S_{\gamma}| \parens{\epsilon_{\sinc} - \frac{2(\dim + 1)}{t^2}}^2.
\end{align}
As we have $t^2 \in \Omega(\dim)$, we expect this to scale like $\bigo{|S_{\gamma}| \epsilon_{\sinc}^2}$. 

\section{Todo}
\localtableofcontents
\subsection{tests}
\begin{itemize}
    \item How would you write out a two hop cooling chain? $\beta_0 \to \beta_1 \to \beta_2$. 
    Want to bound $\norm{\Phi_{\beta_2} \circ \Phi_{\beta_1} \circ \Phi_{\beta_0} - \rho_S(\beta_2)}_1$.
    
    \item Find value of $\beta$ such that $\norm{\rho_S(\beta) - \Phi(\rho_S(\beta))}_1 \geq \epsilon$
