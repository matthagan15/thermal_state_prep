...
use ndarray::{Array2, linalg::kron, ShapeBuilder};
use ndarray_linalg::{Scalar, expm, krylov::R};
use num_complex::Complex64 as c64;

use crate::{RandomInteractionGen, partial_trace, adjoint, thermal_state};
...
impl Channel {
    ...
    pub fn new(
        ...
        let h_tot = kron(&system_hamiltonain, &Array2::<c64>::eye(de)) + kron(&Array2::<c64>::eye(ds), &environment_hamiltonian);
    ...
    pub fn map(
        ...
        partial_trace(&self.total_map(rho_sys, alpha, time), self.dim_sys, self.dim_env)
    }
    ...
    pub fn total_map(
        ...
        let tot = c64::new(0.0, time) * (c64::from_real(alpha)* g + &self.h_tot);
    ...
    pub fn map_k_times(
        ...
        partial_trace(&self.total_map_k_times(rho_sys, alpha, time, num_interactions), self.dim_sys, self.dim_env)
    }
}

mod test {
    use ndarray::{Array2, linalg::kron};
    use ndarray_linalg::{OperationNorm, Scalar};
    use num_complex::Complex64 as c64;

    use crate::{RandomInteractionGen, HamiltonianType, harmonic_oscillator_hamiltonian, thermal_state, perform_fixed_interaction_channel};
    ...
    fn compare_channel_with_known() {
        ...
        let d = h1 - h2;
        println!("difference in first sampled interaction: {:}", d.opnorm_one().unwrap());

        let h_sys = harmonic_oscillator_hamiltonian(20);
        let h_env = harmonic_oscillator_hamiltonian(2);
        let h_tot = kron(&h_sys, &Array2::<c64>::eye(h_env.nrows())) + kron(&Array2::<c64>::eye(h_sys.nrows()), &h_env);
        let b_env = 1.;
        let alpha = 0.001;
        let time = 1000.;
        ...
        for _ in 0..100 {
            ...
            let fn_out = perform_fixed_interaction_channel(&h_tot, &g, &rho_tot, time, h_sys.nrows());
